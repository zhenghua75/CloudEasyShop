...
export default class DeptPicker extends Component {
  constructor(props) {
    super(props);
  ...
  getPickerTitle(){
    const {depts,value} = this.props;
    if(!value){
      return '所有门店';
    }else{
      var options=null;
      var strtitle='';
      if (this.props.optionDeptKey) {
        if (depts && depts.options && depts.options[this.props.optionDeptKey]) {
          options = depts.options[this.props.optionDeptKey];
        }
      }
      if(options){
  ...
  render() {
    ...
    if(options.length==0 || (options.length>0 && options[0].value!=='')){
      options.splice(0,0,{label:'所有门店',value:''});
    }
            ...
            <View style={styles.modalview}>
              <View style={styles.navbar}>
                ...
                <View style={styles.navtitle}>
                  <Text style={styles.navtitletext}allowFontScaling={false}>门店</Text>
                </View>
              </View>
