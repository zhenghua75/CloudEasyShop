...
export default class BatchPicker extends Component {
  ...
  getPickerTitle(){
    const {currentStock,value} = this.props;
    var options=null;
    var strtitle='';
    if (currentStock) {
      options = currentStock.data;
      if (!options || options.length == 0) {
        return '该存货暂无库存';
      }else{
        if(value){
          for (var i in options) {
            if(options[i].Batch==value){
              strtitle=options[i].Batch;
            }
          }
        }
        if(strtitle==''){
  ...
  render() {
    ...
    var createitem=(item,i)=>{
      if(value==item.Batch){
        return (
          <TouchableOpacity key={i} onPress={()=>this.onPressConfirm(item.Batch)}
          style={styles.pickeritem}>
            <View style={styles.pickeritemview}>
              <Text style={[styles.pickeritemtext,styles.pickerselitemtext]} allowFontScaling={false}>{item.Batch}</Text>
        ...
        return (
          ...
          style={styles.pickeritem}>
            <View style={styles.pickeritemview}>
              <Text style={styles.pickeritemtext} allowFontScaling={false}>{item.Batch}</Text>
            </View>
          </TouchableOpacity>
        );
      }
            ...
            <View style={styles.modalview}>
              <View style={styles.navbar}>
                ...
                <View style={styles.navtitle}>
                  <Text style={styles.navtitletext}allowFontScaling={false}>批号</Text>
                </View>
              </View>
