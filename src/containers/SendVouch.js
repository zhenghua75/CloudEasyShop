...
class SendVouch extends Component {
	constructor(props) {
		super(props);
		this.state = {
			tabselected: '1'
	...
	shouldComponentUpdate(nextProps, nextState) {
		isupdate = false;
		var map1 = Immutable.fromJS(nextProps.myVouch['009']);
		var map2 = Immutable.fromJS(this.props.myVouch['009']);
	...
	renderRow(rowData, sectionID, rowID, highlightRow) {
		if (rowData && rowData.Sn) {
			if (this.state.tabselected === '1' && rowData.IsVerify === false) {
				if (rowData.SourceId > 0) {
					return (
						<TouchableOpacity style={[styles.row]} onPress={()=>{Actions.ReadVouch({vouchId:rowData.Id,vouchType:'009'})}}>
	          ...
	          <View style={styles.bottomview}>
				...
				return (
					<TouchableOpacity style={[styles.row]} onPress={()=>{Actions.ReadVerifyVouch({vouchId:rowData.Id,vouchType:'009'})}}>
          ...
          <View style={styles.midview}>
            ...
            <Text style={styles.detailtext} allowFontScaling={false}>发货仓：{rowData.FromWhName}</Text>
            <Text style={styles.detailtext} allowFontScaling={false}>制单人：{rowData.FullName}</Text>
	          <Text style={styles.detailtext} allowFontScaling={false}>发货审核人：{rowData.VerifyFullName}</Text>
          </View>
          <View style={styles.bottomview}>
            <Text style={styles.detailtext} allowFontScaling={false}>{rowData.MakeTime}</Text>
          </View>
	...
	render() {
		...
		if(myVouch && myVouch['009']){
			if (myVouch['009'].vouch && myVouch['009'].vouch.length > 0) {
				dataSource = ds.cloneWithRows(myVouch['009'].vouch);
			} else if (myVouch['009'].vouch) {
        ...
        <View style={styles.lists}>
          <ListView
            ...
            refreshControl={
              <RefreshControl
                refreshing={myVouch['009'].isFetching}
                onRefresh = {
                  this._onRefresh
                }
                tintColor="#7BBFEA"
...
export default connect(mapStateToProps, mapDispatchToProps)(SendVouch);