...
class ProdinVouchAdd extends Component {
  constructor(props) {
    super(props);
  ...
  submitAddVouchAndVouchs() {
    ...
    if (profile.user.whId == null || profile.user.whId == '') {
      ...
      if (this.props.sourceId) {
        vouchData.data[0].Vouch.SourceId = this.props.sourceId;
      }
      let vouchsData = {
        action: 'create',
        data: [{
      ...
      actions.vouchAndVouchs(auth.token.access_token, '006', vouchData, vouchsData,auth.info.api)
  ...
  componentDidMount() {
    ...
    if (isrequest) {
      ...
      actions.vouchAndVouchs(auth.token.access_token, '006', data, datavouchs,auth.info.api);
    }
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(ProdinVouchAdd);