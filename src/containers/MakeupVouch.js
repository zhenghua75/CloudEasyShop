...
class MakeupVouch extends Component {
	constructor(props) {
		super(props);
		this.state = {
			tabselected: '1'
		...
		if (delvid == '' || delvid == '0' || delcode == '' || delcode == '0') {
			...
			Alert.alert('请确认', alertmsg, [{
				...
				onPress: () => {
					...
					actions.vouch(auth.token.access_token, '014', data,auth.info.api);
					this._onRefresh();
				}
			}]);
		}
	...
	shouldComponentUpdate(nextProps, nextState) {
		isupdate = false;
		var map1 = Immutable.fromJS(nextProps.myVouch['014']);
		var map2 = Immutable.fromJS(this.props.myVouch['014']);
	...
	renderRow(rowData, sectionID, rowID, highlightRow) {
		if (rowData && rowData.Sn) {
			if (this.state.tabselected == '1' && rowData.IsVerify == false) {
				return (
					<TouchableOpacity style={[styles.row]} onPress={()=>{Actions.ReadVouch({vouchId:rowData.Id,vouchType:'014'})}}>
          ...
          <View style={styles.bottomview}>
            <Text style={styles.detailtext} allowFontScaling={false}>{rowData.MakeTime}</Text>
				...
				return (
					<TouchableOpacity style={[styles.row]} onPress={()=>{Actions.MakeupProdinReadVouch({vouchId:rowData.Id,vouchType:'014'})}}>
          <View style={styles.topview}>
            <Text style={styles.sn} allowFontScaling={false}>{rowData.Code}</Text>
          </View>
          <View style={styles.horizontalLine}/>
	...
	render() {
		...
		if (myVouch['014'].vouch && myVouch['014'].vouch.length > 0) {
			dataSource = ds.cloneWithRows(myVouch['014'].vouch);
		} else if (myVouch['014'].vouch) {
        ...
        <View style={styles.lists}>
          <ListView
            ...
            refreshControl={
              <RefreshControl
                refreshing={myVouch['014'].isFetching}
                onRefresh = {
                  this._onRefresh
                }
                tintColor="#7BBFEA"
...
export default connect(mapStateToProps, mapDispatchToProps)(MakeupVouch);