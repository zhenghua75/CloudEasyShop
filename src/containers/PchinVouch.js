...
class PchinVouch extends Component {
	constructor(props) {
		super(props);
		this.state = {
			tabselected: '1'
		...
		if (delvid == '' || delvid == '0' || delcode == '' || delcode == '0') {
			...
			Alert.alert('请确认', alertmsg, [{
				...
				onPress: () => {
					...
					actions.vouch(auth.token.access_token, '001', data,auth.info.api);
					this._onRefresh();
				}
			}]);
		}
	...
	shouldComponentUpdate(nextProps, nextState) {
		isupdate = false;
		var map1 = Immutable.fromJS(nextProps.myVouch['001']);
		var map2 = Immutable.fromJS(this.props.myVouch['001']);
	...
	renderRow(rowData, sectionID, rowID, highlightRow) {
		if (rowData && rowData.Sn) {
			if (this.state.tabselected == '1' && rowData.IsVerify == false) {
				return (
					<TouchableOpacity style={[styles.row]} onPress={()=>{Actions.ReadVouch({vouchId:rowData.Id,vouchType:'001'})}}>
          ...
          <View style={styles.midview}>
            <Text style={styles.detailtext} allowFontScaling={false}>采购仓：{rowData.ToWhName}</Text>
          ...
          <View style={styles.bottomview}>
            <Text style={styles.detailtext} allowFontScaling={false}>{rowData.MakeTime}</Text>
				...
				return (
					<TouchableOpacity style={[styles.row]} onPress={()=>{Actions.ReadVerifyVouch({vouchId:rowData.Id,vouchType:'001'})}}>
          ...
          <View style={styles.midview}>
            <Text style={styles.detailtext} allowFontScaling={false}>采购仓：{rowData.ToWhName}</Text>
            <Text style={styles.detailtext} allowFontScaling={false}>制单人：{rowData.FullName}</Text>
            <Text style={styles.detailtext} allowFontScaling={false}>采购审核人：{rowData.VerifyFullName}</Text>
          </View>
          <View style={styles.bottomview}>
            <Text style={styles.detailtext} allowFontScaling={false}>{rowData.MakeTime}</Text>
          </View>
	...
	render() {
		...
		if (myVouch['001'].vouch && myVouch['001'].vouch.length > 0) {
			dataSource = ds.cloneWithRows(myVouch['001'].vouch);
		} else if (myVouch['001'].vouch) {
        ...
        <View style={styles.lists}>
          <ListView
            ...
            refreshControl={
              <RefreshControl
                refreshing={myVouch['001'].isFetching}
                onRefresh = {
                  this._onRefresh
                }
                tintColor="#7BBFEA"
...
export default connect(mapStateToProps, mapDispatchToProps)(PchinVouch);