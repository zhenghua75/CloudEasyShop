...
class ReceiveVouch extends Component {
	constructor(props) {
		super(props);
		this.state = {
			tabselected: '1'
	...
	shouldComponentUpdate(nextProps, nextState) {
		isupdate = false;
		var map1 = Immutable.fromJS(nextProps.myVouch['003']);
		var map2 = Immutable.fromJS(this.props.myVouch['003']);
	...
	renderRow(rowData, sectionID, rowID, highlightRow) {
		if (rowData && rowData.Sn) {
			if (this.state.tabselected == '1' && rowData.IsVerify == false) {
				return (
					<TouchableOpacity style={[styles.row]} onPress={()=>{Actions.ReadVouch({vouchId:rowData.Id,vouchType:'003',fromWhName:rowData.FromWhName})}}>
          <View style={styles.topview}>
            <Text style={styles.sn} allowFontScaling={false}>{rowData.Code}</Text>
          </View>
          <View style={styles.horizontalLine}/>
          <View style={styles.midview}>
            <Text style={styles.detailtext} allowFontScaling={false}>收货仓：{rowData.ToWhName}</Text>
          ...
          <View style={styles.bottomview}>
            <Text style={styles.detailtext} allowFontScaling={false}>{rowData.MakeTime}</Text>
				...
				return (
					<TouchableOpacity style={[styles.row]} onPress={()=>{Actions.ReadVerifyVouch({vouchId:rowData.Id,vouchType:'003',fromWhName:rowData.FromWhName})}}>
          ...
          <View style={styles.midview}>
            ...
            <Text style={styles.detailtext} allowFontScaling={false}>发货仓：{rowData.FromWhName}</Text>
            <Text style={styles.detailtext} allowFontScaling={false}>制单人：{rowData.FullName}</Text>
            <Text style={styles.detailtext} allowFontScaling={false}>收货审核人：{rowData.VerifyFullName}</Text>
          </View>
          <View style={styles.bottomview}>
            <Text style={styles.detailtext} allowFontScaling={false}>{rowData.MakeTime}</Text>
          </View>
	...
	render() {
		...
		if(myVouch && myVouch['003']){
			if (myVouch['003'].vouch && myVouch['003'].vouch.length > 0) {
				dataSource = ds.cloneWithRows(myVouch['003'].vouch);
			} else if (myVouch['003'].vouch) {
        ...
        <View style={styles.lists}>
          <ListView
            ...
            refreshControl={
              <RefreshControl
                refreshing={myVouch['003'].isFetching}
                onRefresh = {
                  this._onRefresh
                }
                tintColor="#7BBFEA"
...
export default connect(mapStateToProps, mapDispatchToProps)(ReceiveVouch);