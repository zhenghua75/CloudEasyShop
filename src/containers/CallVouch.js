...
class CallVouch extends Component {
  constructor(props) {
    super(props);
    this.state = {
      tabselected: '1'
    ...
    if (delvid === '' || delvid === '0' || delcode === '' || delcode === '0') {
      ...
      Alert.alert('请确认', alertmsg, [{
        ...
        onPress: () => {
          ...
          actions.vouch(auth.token.access_token, '012', data,auth.info.api);
          this._onRefresh();
        }
      }]);
    }
  ...
  shouldComponentUpdate(nextProps, nextState) {
    isupdate = false;
    var map1 = Immutable.fromJS(nextProps.myVouch['012']);
    var map2 = Immutable.fromJS(this.props.myVouch['012']);
  ...
  renderRow(rowData, sectionID, rowID, highlightRow) {
    if (rowData && rowData.Sn) {
      if (this.state.tabselected === '1' && rowData.IsVerify === false) {
        return (
          <TouchableOpacity style={[styles.row]} onPress={()=>{Actions.ReadVouch({vouchId:rowData.Id,vouchType:'012'})}}>
        ...
        return (
          <TouchableOpacity style={[styles.row]} onPress={()=>{Actions.ReadVerifyVouch({vouchId:rowData.Id,vouchType:'012'})}}>
          <View style={styles.topview}>
            <Text style={styles.sn} allowFontScaling={false}>{rowData.Code}</Text>
          </View>
          <View style={styles.horizontalLine}/>
          <View style={styles.midview}>
            ...
            <Text style={styles.detailtext} allowFontScaling={false}>发货仓：{rowData.FromWhName}</Text>
            <Text style={styles.detailtext} allowFontScaling={false}>叫货人：{rowData.FullName}</Text>
            <Text style={styles.detailtext} allowFontScaling={false}>叫货审核人：{rowData.VerifyFullName}</Text>
          </View>
          <View style={styles.bottomview}>
            <Text style={styles.detailtext} allowFontScaling={false}>{rowData.MakeTime}</Text>
          </View>
  ...
  render() {
    ...
    if(myVouch && myVouch['012']){
      if (myVouch['012'].vouch && myVouch['012'].vouch.length > 0) {
        dataSource = ds.cloneWithRows(myVouch['012'].vouch);
      } else if (myVouch['012'].vouch) {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.lists}>
          <ListView
              ...
              refreshControl={
              <RefreshControl
                  refreshing={myVouch['012'].isFetching}
                  onRefresh = {
                    this._onRefresh
                  }
                  tintColor="#7BBFEA"
...
export default connect(mapStateToProps, mapDispatchToProps)(CallVouch);