...
class CallVouchAdd extends Component {
  constructor(props) {
    super(props);
    let date = new Date();
  ...
  submitAddVouchAndVouchs() {
    ...
    if (this.state.fromwhid == null || this.state.fromwhid == '') {
      Alert.alert('提示', '请选择生产仓库', [{
        text: '确定'
      }]);
      ...
      actions.vouchAndVouchs(auth.token.access_token, '012', vouchData, vouchsData,auth.info.api)
  ...
  componentDidMount() {
    ...
    if (isrequest) {
      ...
      actions.vouchAndVouchs(auth.token.access_token, '012', data, datavouchs,auth.info.api);
    }
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(CallVouchAdd);