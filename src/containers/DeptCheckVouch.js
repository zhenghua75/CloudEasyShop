...
class DeptCheckVouch extends Component {
  constructor(props) {
    super(props);
    this.state = {
      tabselected: '1',
    ...
    if (delvid === '' || delvid === '0' || delcode === '' || delcode === '0') {
      ...
      Alert.alert('请确认', alertmsg, [{
        ...
        onPress: () => {
          ...
          actions.vouch(auth.token.access_token, '016', data,auth.info.api)
  ...
  shouldComponentUpdate(nextProps, nextState) {
    isupdate = false;
    var map1 = Immutable.fromJS(nextProps.myVouch['016']);
    var map2 = Immutable.fromJS(this.props.myVouch['016']);
  ...
  renderRow(rowData, sectionID, rowID, highlightRow) {
    if (rowData && rowData.Sn) {
      let busname='';
      if(rowData.BusType==='023'){
        busname='产成品';
      }else if(rowData.BusType==='024'){
        busname='半成品';
      }else if(rowData.BusType==='025'){
        busname='原材料';
      }
      if (this.state.tabselected === '1' && rowData.IsVerify === false) {
        return (
          <TouchableOpacity style={[styles.row]} onPress={()=>{Actions.ReadCheckVouch({vouchId:rowData.Id,vouchType:'016',isVerify:false,busType:rowData.BusType})}}>
          ...
          <View style={styles.midview}>
            ...
            <Text style={styles.detailtext} allowFontScaling={false}>业务类型：{busname}</Text>
        ...
        return (
          <TouchableOpacity style={[styles.row]} onPress={()=>{Actions.ReadCheckVouch({vouchId:rowData.Id,vouchType:'016',isVerify:true,busType:rowData.BusType})}}>
          <View style={styles.topview}>
            <Text style={styles.sn} allowFontScaling={false}>{rowData.Code}</Text>
          </View>
          <View style={styles.horizontalLine}/>
  ...
  render() {
    ...
    if(myVouch && myVouch['016']){
      if (myVouch['016'].vouch && myVouch['016'].vouch.length > 0) {
        dataSource = ds.cloneWithRows(myVouch['016'].vouch);
      } else if (myVouch['016'].vouch) {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.lists}>
          <ListView
              ...
              refreshControl={
              <RefreshControl
                  refreshing={myVouch['016'].isFetching}
                  onRefresh = {
                    this._onRefresh
                  }
                  tintColor="#7BBFEA"
...
export default connect(mapStateToProps, mapDispatchToProps)(DeptCheckVouch);